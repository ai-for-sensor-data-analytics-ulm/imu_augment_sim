...
from .constants import (
    INTRINSIC_EULER_ORDER,
    QUATERNION_AXES,
    EULER_JUMP_THRESHOLD_DEG,
    GLITCH_DISTANCE_THRESHOLD,
)
...
def remove_jumps_from_euler_angle(
    ...
    jump_indices = np.argwhere(np.abs(t_diff) > EULER_JUMP_THRESHOLD_DEG).flatten()
...
def calculate_distribution_parameters_from_data(
    ...
    variance_scaling : float, optional
        Multiplicative factor applied to the estimated covariance matrices
        (``cov_deltas`` and ``cov_offsets``). Defaults to ``1.0`` (no scaling).
    ...
    for label in set(labels):
        ...
        for imu in imu_names:
            ...
            distribution_parameters[label][imu] = {
                ...
                'cov_deltas': np.cov(deltas) * variance_scaling,
                'mean_offsets': np.mean(offsets, axis=1),
                'cov_offsets': np.cov(offsets) * variance_scaling,
            }
...
def unify_quaternions(quat_data, imu_names):
    ...
    for imu_name in imu_names:
        ...
        if max(distances) > GLITCH_DISTANCE_THRESHOLD:
            quat = fix_glitches(quat)

            distances = find_glitches(quat)
            if max(distances) > GLITCH_DISTANCE_THRESHOLD:
                logger.warning("IMU Position %s", imu_name)
