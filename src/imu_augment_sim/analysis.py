...
def read_sto_file(filename, sep=None):
    ...
    -------
    pandas.DataFrame
        Parsed data table. The ``.sto`` header section is consumed while
        parsing but is not part of the returned DataFrame.
    """
    ...
    # ``errors='raise'`` surfaces malformed (non-numeric) columns instead of
    # silently leaving them as strings, which would fail later in unclear ways.
    measured_data = pd.DataFrame(data=data, columns=labels).astype('float', errors='raise')
...
def __adapt_analysis_settings(
    ...
    -------
    Path
        Path to the adapted settings file. The file is written next to the
        analysis results so the versioned template is never modified in place.
    """
    ...
    adapted_settings_path = Path(analyzer_results_path) / "analyzer_settings.xml"
    tree.write(adapted_settings_path)
    return adapted_settings_path
...
def perform_analysis(analyzer_settings_path, model_path, analyzer_results_path, coordinates_file_name):
    ...
    """
    adapted_settings_path = __adapt_analysis_settings(
        analyzer_settings_path, model_path, analyzer_results_path, coordinates_file_name
    )
    analysis_tool = osim.AnalyzeTool(str(adapted_settings_path))
    analysis_tool.run()
    return
...
def load_analysis_data(analyzer_results_path):
    ...
    -------
    dict
        Dictionary with keys ``positions``, ``orientations`` and ``joint_angles``.
    """
    ...
    return {'positions': positions, 'orientations': orientations, 'joint_angles': joint_angles}
