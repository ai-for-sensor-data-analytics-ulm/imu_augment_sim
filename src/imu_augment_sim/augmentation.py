...
def add_jumps_to_euler(angle_series: np.ndarray) -> np.ndarray:
    ...
    """
    angle_series = np.where(angle_series < -HALF_TURN_DEG, angle_series + FULL_TURN_DEG, angle_series)
    return np.where(angle_series > HALF_TURN_DEG, angle_series - FULL_TURN_DEG, angle_series)
